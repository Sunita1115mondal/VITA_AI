...
import { Sparkles, Trophy, Calendar, Moon, Sun, Dumbbell, ArrowRight, RefreshCw, Quote, Heart, Volume2, Loader2 } from 'lucide-react';
import { AnalysisResult, CoachInsight } from '../types';
  ...
  const playCoachAudio = async () => {
    ...
    try {
      ...
      const script = `
        ...
        For your exercise today, I recommend ${String(insight.exercisePlan.type)} for ${String(insight.exercisePlan.duration)}.
        Let's get moving!
      `;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 animate-in fade-in">
        ...
        <p className="mt-4 text-slate-500 font-medium animate-pulse">Your Personal Health Coach is thinking...</p>
      </div>
    );
  }
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl md:text-4xl font-extrabold leading-tight mb-4 flex gap-3">
             <Quote className="flex-shrink-0 fill-white/20 stroke-none" size={40} />
             {String(insight.motivationalMessage)}
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                 <div className="flex-1">
                    <div className="text-3xl font-black text-slate-900 mb-1">{String(insight.exercisePlan.type)}</div>
                    <div className="flex items-center gap-3 text-sm font-medium text-slate-500 mb-3">
                       <span className="flex items-center gap-1"><Calendar size={14} /> {String(insight.exercisePlan.duration)}</span>
                       <span className={`px-2 py-0.5 rounded text-xs uppercase ${
                         String(insight.exercisePlan.intensity) === 'High' ? 'bg-red-100 text-red-700' : 
                         String(insight.exercisePlan.intensity) === 'Medium' ? 'bg-yellow-100 text-yellow-700' : 
                         'bg-emerald-100 text-emerald-700'
                       }`}>{String(insight.exercisePlan.intensity)} Intensity</span>
                    </div>
                    <p className="text-slate-600">{String(insight.exercisePlan.description)}</p>
                 </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
            <h3 className="text-sm font-bold text-indigo-900 uppercase tracking-wider mb-4">Daily Reminders</h3>
            <ul className="space-y-3">
              {insight.dailyReminders.map((reminder, idx) => (
                <li key={idx} className="flex items-start gap-3 bg-white p-3 rounded-xl border border-indigo-100 shadow-sm">
                  <div className="w-5 h-5 rounded-full border-2 border-indigo-200 mt-0.5 flex-shrink-0"></div>
                  <span className="text-slate-700 text-sm font-medium">{String(reminder)}</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                   ...
                   <span className="text-white font-bold">{insight.sleepTracking.hours} hrs</span>
                </div>
                <div className="w-full bg-slate-700 rounded-full h-2 mb-2">
                   <div 
                     className="bg-indigo-400 h-2 rounded-full" 
                     style={{ width: String(insight.sleepTracking.quality) === 'Good' ? '85%' : String(insight.sleepTracking.quality) === 'Fair' ? '60%' : '40%' }}
                   ></div>
                </div>
                <p className="text-xs text-slate-400">{String(insight.sleepTracking.insight)}</p>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                   ...
                   <span className="text-white font-bold">{String(insight.stressTracking.level)}</span>
                </div>
                <div className="w-full bg-slate-700 rounded-full h-2 mb-2">
                   <div 
                     className="bg-rose-500 h-2 rounded-full" 
                     style={{ width: String(insight.stressTracking.level) === 'High' ? '90%' : String(insight.stressTracking.level) === 'Medium' ? '50%' : '20%' }}
                   ></div>
                </div>
                <p className="text-xs text-slate-400">{String(insight.stressTracking.insight)}</p>
              </div>
            </div>
