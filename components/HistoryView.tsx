...
import { FileDown, Download, Brain, TrendingUp, AlertTriangle, Calendar, Activity, CheckCircle } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Latest (Today)</span>
            <Activity className="text-blue-500" size={18} />
          </div>
          <div className="flex items-baseline gap-2">
            <h3 className="text-3xl font-bold text-slate-900">{latest.heartRate}</h3>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">10-Day Avg HR</span>
         ...
         <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-6 rounded-2xl text-white shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-indigo-200 uppercase tracking-wider">AI Trend Insight</span>
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-slate-900">Heart Rate</th>
                <th className="px-6 py-4 font-semibold text-slate-900">BP (mmHg)</th>
                <th className="px-6 py-4 font-semibold text-slate-900">Status</th>
                <th className="px-6 py-4 font-semibold text-slate-900">AI Insight</th>
              </tr>
            </thead>
              ...
              {DATA.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 font-medium">
                    {record.date} <span className="text-slate-400 text-xs ml-1">{record.time}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-mono text-slate-900">{record.heartRate}</span> BPM
                  </td>
                  <td className="px-6 py-4">
                     <span className="font-mono text-slate-900">{record.bloodPressure}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold ${
                      record.riskLevel === 'Low' ? 'bg-emerald-100 text-emerald-700' :
                      record.riskLevel === 'Moderate' ? 'bg-amber-100 text-amber-700' :
                      'bg-rose-100 text-rose-700'
                    }`}>
                      {record.riskLevel === 'Low' ? <CheckCircle size={10} /> : <AlertTriangle size={10} />}
                      {record.riskLevel}
                    </span>
                  </td>
