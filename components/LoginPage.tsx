...
import { Activity, Mic, Lock, Mail, ArrowRight, UserPlus, Loader2 } from 'lucide-react';
...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
    ...
    recognition.onresult = (event: any) => {
      ...
      if (text.includes('login') || text.includes('log in')) {
        ...
        if (btn) {
           ...
           setTimeout(() => btn.classList.remove('ring-4', 'ring-blue-200'), 500);
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col items-center justify-center p-4 font-sans relative">
      ...
      <div 
        className={`fixed top-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md border border-slate-200 shadow-xl px-6 py-3 rounded-full flex items-center gap-3 transition-all duration-500 z-50 ${isListening ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}
        role="status"
        aria-live="polite"
      >
        <div className="w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.5)]"></div>
        <span className="text-slate-700 font-semibold text-sm">Listening... Say "Login" or "Create Account"</span>
      </div>
      ...
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl border border-white/50 overflow-hidden relative animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="p-8 sm:p-10">
          ...
          <div className="flex flex-col items-center text-center mb-10">
            ...
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">VITA-AI</h1>
            <p className="text-slate-500 mt-2 font-medium">Health Prediction System</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-end">
                <button type="button" className="text-sm font-semibold text-blue-600 hover:text-blue-700 hover:underline px-1 transition-colors">
                  Forgot Password?
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            >
              <UserPlus size={20} className="text-slate-400 group-hover:text-blue-500 transition-colors" />
              Create Account
            </button>
            ...
            <div className="pt-2">
                ...
                className={`w-full py-3 rounded-xl font-medium text-sm transition-all flex items-center justify-center gap-2 ${isListening ? 'bg-red-50 text-red-600 border border-red-100' : 'text-slate-500 hover:bg-slate-50'}`}
                aria-label={isListening ? "Stop Voice Control" : "Enable Voice Control"}
                >
                {isListening ? (
                    <><Mic size={16} className="animate-pulse" /> Voice Listening Active</>
                ) : (
