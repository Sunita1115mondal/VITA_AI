...
const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  const handleReadAloud = async () => {
    ...
    try {
      ...
      const textToRead = `Analysis complete. Risk level is ${
        result.riskLevel
      }. Heart rate estimated at ${
        result.vitalMetrics.heartRate
      } beats per minute. ${
        result.reasoning
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Risk Level Card */}
        <div
          className={`p-6 rounded-2xl border flex items-center justify-between ${getRiskColor(
            result.riskLevel
          )}`}
        >
          <div>
        ...
        <div className="p-6 rounded-2xl border border-slate-200 bg-white flex items-center justify-between shadow-sm">
          ...
          <div className="relative w-16 h-16 flex items-center justify-center">
            ...
            <HeartPulse
              size={20}
              className={`absolute ${getScoreColor(result.fitnessScore)}`}
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl p-6 text-white shadow-lg border border-slate-800 relative overflow-hidden">
        ...
        {isSonifying && (
          <div className="absolute inset-0 z-0 flex items-center justify-center">
            <div
              className="w-full h-full bg-blue-500/10 animate-pulse"
              style={{
                animationDuration: `${
                  60000 / (result.vitalMetrics.heartRate || 75)
                }ms`,
              }}
            ></div>
          </div>
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm max-w-md">
              ...
              <span className="text-xs opacity-70">
                Low pitch = Low HR • Fast tempo = Fast HR • Smooth tone = Normal
              </span>
            </p>
